...
def main(req):
    ...
    remote_control = req.params.get('remotecontrol')
    if not remote_control:
        try:
            ...
            remote_control = req_body.get('remotecontrol')
            ...
            props.update(remote_control = remote_control)
            registry_manager.send_c2d_message(DEVICE_ID, data, properties=props)
            
        return func.HttpResponse(f"remote_control, {remote_control}. This HTTP Remotecontrol triggered function executed successfully.")
    else:
